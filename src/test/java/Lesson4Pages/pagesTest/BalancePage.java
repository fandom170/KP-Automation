package Lesson4Pages.pagesTest;


import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.FindBy;
import org.openqa.selenium.support.PageFactory;

public class BalancePage {
    WebDriver driver;

    public String funBalanceValue = "1000";
    public String funBalanceNote = "Test_0001";
    public String realBalanceValue = "2222";
    public String realBalanceNote = "Test_0002";
    public String bonusBalanceValue = "3333";
    public String bonusBalanceNote = "Test_0003";

    @FindBy(xpath = "//input [contains (@name, '__amount')]")
    WebElement userBalanceAmountInput;

    @FindBy(xpath = "//textarea [contains(@id, '_notes')]")
    WebElement userBalanceNotesTextarea;

    //@FindBy (xpath = "//select[contains(@id, '__account')]")
    //WebElement userBalanceAccountSelect;

    @FindBy(name = "button_save")
    WebElement userBalanceSaveButton;

    public BalancePage(WebDriver driver) {
        this.driver = driver;
        PageFactory.initElements(driver, this);
    }

    public void setUserBalanceAmount(String usBal) {
        userBalanceAmountInput.sendKeys(usBal);
    }

    public void setUserBalanceNotes(String usNotes) {
        userBalanceNotesTextarea.sendKeys(usNotes);
    }

    public void clickSaveBalance() {
        userBalanceSaveButton.click();
    }

    public void balanceFunEnter(String usBal, String usNotes) {
        //User balance
        this.setUserBalanceAmount(usBal);
        //Notes
        this.setUserBalanceNotes(usNotes);
        //Click save
        this.clickSaveBalance();

    }

    public void balanceRealEnter(String usBal, String usNotes) {

        //User balance
        this.setUserBalanceAmount(usBal);
        //Notes
        this.setUserBalanceNotes(usNotes);
        //Click save
        this.clickSaveBalance();

    }

    public void balanceBonusEnter(String usBal, String usNotes) {
        //User balance
        this.setUserBalanceAmount(usBal);
        //Notes
        this.setUserBalanceNotes(usNotes);
        //Click save
        this.clickSaveBalance();

    }
}
